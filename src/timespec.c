 ...
 * </PRE>
 *
 * Furthermore, any timespec structure processed or returned by library
 * functions is normalised according to the rules in timespec_normalise().
 */
...
/** \fn struct timespec timespec_clamp(struct timespec ts, struct timespec min,
 * struct timespec max)
 ...
 *  \brief Returns (1, 0, -1) if ts1 is (greater than, equal to, less than) to
 * ts2.
 */
...
#define TEST_BINOP(func, ts1_sec, ts1_nsec, ts2_sec, ts2_nsec, expect_sec,     \
                   expect_nsec)                                                \
    {                                                                          \
...
#define TEST_TRINOP(func, ts1_sec, ts1_nsec, ts2_sec, ts2_nsec, ts3_sec,       \
                    ts3_nsec, expect_sec, expect_nsec)                         \
    {                                                                          \
        struct timespec ts1 = {.tv_sec = ts1_sec, .tv_nsec = ts1_nsec};        \
        struct timespec ts2 = {.tv_sec = ts2_sec, .tv_nsec = ts2_nsec};        \
        ...
        if (got.tv_sec != expect_sec || got.tv_nsec != expect_nsec) {          \
            printf(#func "({%ld, %ld}, {%ld, %ld}, {%ld, %ld}) returned "      \
                         "wrong values\n",                                     \
                   (long)(ts1_sec), (long)(ts1_nsec), (long)(ts2_sec),         \
...
#define TEST_TO_DOUBLE(ts_sec, ts_nsec, expect)                                \
    {                                                                          \
        ...
        if (got != expect) {                                                   \
            printf("%s:%d: timespec_to_double({%ld, %ld}) returned wrong "     \
                   "value\n",                                                  \
                   __FILE__, __LINE__, (long)(ts_sec), (long)(ts_nsec));       \
            printf("    Expected: %f\n", (double)(expect));                    \
            printf("    Got:      %f\n", got);                                 \
...
#define TEST_TO_TIMEVAL(ts_sec, ts_nsec, expect_sec, expect_usec)              \
    {                                                                          \
        ...
        if (got.tv_sec != expect_sec || got.tv_usec != expect_usec) {          \
            printf("%s:%d: timespec_to_timeval({%ld, %ld}) returned wrong "    \
                   "values\n",                                                 \
                   __FILE__, __LINE__, (long)(ts_sec), (long)(ts_nsec));       \
            printf("    Expected: {%ld, %ld}\n", (long)(expect_sec),           \
                   (long)(expect_usec));                                       \
